...
class LocalSearchServiceTest: AbstractServiceTest() {
    init {
        ...
        val searchHistoryRepository: SearchHistoryRepository by inject()

        Given("장소 검색") {
            When("'곱창' 키워드로 검색") {
                val query = "곱창"

                every { searchHistoryRepository.findByKeyword(any()) } answers { null }
                every { searchHistoryRepository.save(any()) } answers { SearchHistory(keyword = query, views = 1) }
